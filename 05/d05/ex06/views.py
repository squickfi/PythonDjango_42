    ...
    if not request.method == 'POST':
        ...
        try:
            with conn.cursor() as curs:
                curs.execute(select)
                movies = curs.fetchall()
            context = {'form': ex06.forms.UpdateForm(choices=(
        ...
        try:
            ...
            print(e)
        data = ex06.forms.UpdateForm(choices, request.POST)
        update_str = """
            UPDATE ex06_movies SET opening_crawl = %s, updated = NOW() WHERE title = %s
        """
        if data.is_valid():
            try:
                with conn.cursor() as curs:
                    curs.execute(update_str, [data.cleaned_data['opening_crawl'], data.cleaned_data['film_to_update']])
                conn.commit()
            except Exception as e:
                print(e)
